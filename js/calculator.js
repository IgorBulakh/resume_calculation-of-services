...
function calculateSum() {
  ...
  const websiteReseption = document.querySelector(
    "#calculator-form-input-reseption input:checked"
  );
  ...
  const websiteReseptionValue = convertRseptionOptionToPrice(
...
function renderSum(sum) {
  const costElement = document.querySelector(".calculator-form-title-cost");
...
function convertRseptionOptionToPrice(option) {
  if (option === "yes") {
    return 500;
  }
  return 0;
